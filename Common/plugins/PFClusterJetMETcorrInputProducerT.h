#ifndef JetMETCorrections_Type1MET_PFClusterJetMETcorrInputProducer_h
#define JetMETCorrections_Type1MET_PFClusterJetMETcorrInputProducer_h

/** \class PFClusterJetMETcorrInputProducer
 *
 * Produce Type 1 + 2 MET corrections corresponding to differences
 * between raw PFClusterJets and PFClusterJets with jet energy corrections (JECs) applied
 *
 */
...
namespace PFClusterJetMETcorrInputProducer_namespace {
  template <typename T>
  class InputTypeCheckerT {
  public:
    void operator()(const T&) const {}  // no type-checking needed for reco::PFClusterJet input
...
class PFClusterJetMETcorrInputProducerT final : public edm::global::EDProducer<> {
public:
  explicit PFClusterJetMETcorrInputProducerT(const edm::ParameterSet& cfg)
  ...
  ~PFClusterJetMETcorrInputProducerT() override {}
...
private:
  void produce(edm::StreamID, edm::Event& evt, const edm::EventSetup& es) const override {
    ...
    if (!srcMET_.label().empty()) {
      ...
      if (met->size() != 1)
        throw cms::Exception("PFClusterJetMETcorrInputProducer::produce")
    ...
    for (int jetIndex = 0; jetIndex < numJets; ++jetIndex) {
      ...
      const PFClusterJetMETcorrInputProducer_namespace::InputTypeCheckerT<T> checkInputType{};
      checkInputType(rawJet);

      const PFClusterJetMETcorrInputProducer_namespace::RawJetExtractorT<T> rawJetExtractor;
  ...
  edm::EDGetTokenT<reco::JetCorrector> offsetCorrToken_;  // e.g. 'ak5PFClusterJetL1Fastjet'
  edm::InputTag jetCorrLabel_;
  edm::EDGetTokenT<reco::JetCorrector>
      jetCorrToken_;  // e.g. 'ak5PFClusterJetL1FastL2L3' (MC) / 'ak5PFClusterJetL1FastL2L3Residual' (Data)
