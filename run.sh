...
if [ ! -f step7.done ]; then
  echo "jet_l2_correction_x -input histogram_ak4puppil1_step6.root -algs ak4puppil1 -era ERA -output l2p3.root -outputDir ./ -makeCanvasVariable AbsCorVsJetPt:JetEta -l2l3 true -batch true -histMet median -l2pffit Standard+Gaussian -maxFitIter 30 -ptclipfit false" > step7.sh
  source step7.sh && touch step7.done
fi

if [ ! -f step8.done ]; then
  echo "mkdir -p plots_step8 && jet_correction_analyzer_x -evtmax 0 -inputFilename jraNTuples/PU_jec.root -algs ak4puppil1 -drmax 0.1 -L1FastJet false -useweight false -path ./ -era ERA -levels 2 -outputDir ./plots_step8 -nbinsrelrsp 200 -relrspmin 0.0 -relrspmax 5.0 -nrefmax 20" > step8.sh
  source step8.sh && touch step8.done
fi

if [ ! -f step9.done ]; then
  echo "jet_draw_closure_x -path plots_step8 -filename Closure_ak4puppil1 -histMet median -outputDir plots_step9 -draw_guidelines true -doPt true -doEta true -doRatioPt false -doRatioEta false" > step9.sh
