#include <NTupleAnalysis/JMETrigger/interface/JMETriggerAnalysisDriverJES.h>
...
JMETriggerAnalysisDriverJES::JMETriggerAnalysisDriverJES(const std::string& tfile, const std::string& ttree, const std::string& outputFilePath, const std::string& outputFileMode)
  : JMETriggerAnalysisDriverJES(outputFilePath, outputFileMode) {

  setInputTTree(tfile, ttree);
}

JMETriggerAnalysisDriverJES::JMETriggerAnalysisDriverJES(const std::string& outputFilePath, const std::string& outputFileMode)
...
void JMETriggerAnalysisDriverJES::bookHistograms_Jets(const std::string& dir, const std::string& jetType, const std::vector<std::string>& matchLabels){
  ...
  for(auto const& matchLabel : matchLabels){
    ...
    addTH2D(dirPrefix+jetType+"_MatchedTo"+matchLabel+"_pt__vs__"+matchLabel+"_pt", binEdges_pt, binEdges_pt);
    addTH2D(dirPrefix+jetType+"_MatchedTo"+matchLabel+"_eta__vs__"+matchLabel+"_eta", binEdges_eta, binEdges_eta);
    addTH1D(dirPrefix+jetType+"_MatchedTo"+matchLabel+"_pt_over"+matchLabel, binEdges_response);
    addTH2D(dirPrefix+jetType+"_MatchedTo"+matchLabel+"_pt_over"+matchLabel+"__vs__"+matchLabel+"_pt", binEdges_response, binEdges_pt);
    addTH2D(dirPrefix+jetType+"_MatchedTo"+matchLabel+"_pt_over"+matchLabel+"__vs__"+matchLabel+"_eta", binEdges_response, binEdges_eta);

    addTH3D(dirPrefix+jetType+"_MatchedTo"+matchLabel+"_pt_over"+matchLabel+"__vs__pt__vs__eta", binEdges_response, binEdges_pt, binEdges_eta);
  ...
  for(auto const& fhDataMatch : fhData.matches){
    ...
    for(auto const jetIdx : jetIndices){
      ...
      for(size_t idxMatch=0; idxMatch<v_match_pt->size(); ++idxMatch){
        ...
        auto const dR2(utils::deltaR2(v_eta->at(jetIdx), v_phi->at(jetIdx), v_match_eta->at(idxMatch), v_match_phi->at(idxMatch)));
        if(dR2 < dR2min){
          dR2min = dR2;
    ...
    for(auto const jetIdx : jetIndices){
      ...
      if(hasMatch){
        ...
        H2(dirPrefix+fhData.jetCollection+"_MatchedTo"+matchLabel+"_pt__vs__"+matchLabel+"_pt")->Fill(jetPt, jetMatchPt);
        H2(dirPrefix+fhData.jetCollection+"_MatchedTo"+matchLabel+"_eta__vs__"+matchLabel+"_eta")->Fill(jetEta, jetMatchEta);
        ...
        if(jetMatchPt != 0.){
          ...
          H2(dirPrefix+fhData.jetCollection+"_MatchedTo"+matchLabel+"_pt_over"+matchLabel+"__vs__"+matchLabel+"_pt")->Fill(jetPtRatio, jetMatchPt);
          H2(dirPrefix+fhData.jetCollection+"_MatchedTo"+matchLabel+"_pt_over"+matchLabel+"__vs__"+matchLabel+"_eta")->Fill(jetPtRatio, jetMatchEta);

          H3(dirPrefix+fhData.jetCollection+"_MatchedTo"+matchLabel+"_pt_over"+matchLabel+"__vs__pt__vs__eta")->Fill(jetPtRatio, jetPt, jetEta);
